...
- `requestFee()` no longer exists. Contracts pay `quoteFee(callbackGasLimit)` in the requesting transaction; off-chain senders quote with `quoteFeeAt(callbackGasLimit, latestBlock.baseFeePerGas)` plus a buffer, for example through `quoteRequestFee`, and must never quote `quoteFee` through `eth_call`.
- Payment is `msg.value >= fee` instead of an exact match; underpayment reverts with `IncorrectFee(expected, actual)` and the excess becomes refund credit of the refund address, withdrawable with `withdrawRefundCredit`. Consumers that forward a player's payment should forward all of it rather than checking for equality. `RandomnessRequested` now carries `feePaid`.
- `EpochProtocolConfiguration.requestFee` is renamed `initialMinFee` (same value, position and encoding; replay inputs need the key renamed, hashes are unchanged), matching the new `initialMinFee()` getter.
- Replay takes the per-epoch signer catalog in `epoch.catalog.signers` (`signersAt(epochId)` or `CatalogScheduled` history) instead of assuming the initial catalog; `replayCoordinator` no longer requires it to equal `configuration.catalogHash`.
- Epoch attestations may be up to 240 seconds old at publication (was 120); `MAX_ATTESTATION_AGE` is exported from `/epoch`.
- An expiry refund pays `feePaid × requestRefundBps / 10000`, so it can be below the full fee for requests created after the owner lowers `refundBps` (never below 50%).
- Fulfillments can arrive in batches of up to 16 (`fulfillRandomnessBatch`, `FulfillmentSkipped`); indexers must key on per-request events, not on one 452-byte calldata per transaction.
- `renounceOwnership` reverts with `RenounceDisabled` on both proxies.
- `examples/DiceConsumer.sol` drops its `WrongFee` check and forwards `msg.value`; the coordinator ABI adds `quoteFee`, `quoteFeeAt`, `pricing`, `setPricing`, `requestFeePaid`, `requestRefundBps`, `setRefundBps`, `initialMinFee`, `fulfillRandomnessBatch` and the `PricingChanged`, `RefundBpsChanged`, `FeeOverpaymentCredited` and `FulfillmentSkipped` events; the registry ABI adds `scheduleCatalog`, `catalogHashAt`, `signersAt`, `MAX_ATTESTATION_AGE` and `CatalogScheduled`.
- Deployment addresses, implementation code hashes and configuration hashes change with the testnet upgrade and the mainnet deployment; take them from the manifests, not from a 0.1.x integration.
