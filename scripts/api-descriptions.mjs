  ...
  },
  {
    ...
    types: {
      'EpochEntropy.Epoch': {
        ...
        fields: {
          ...
          catalogHash: 'Hash of the catalog in force for the epoch, the one `catalogAt(epochId)` returns.',
          anchorHash: 'Hash of block `epochStart - 1`, which selects the source.',
          source: 'Committed source slot in the epoch\'s catalog: the selected slot or a fallback. The recipe is the catalog\'s recipe at that slot.',
          queryHash: '`keccak256` of the canonical request string of the committed recipe.',
      ...
      'EpochEntropy.Selection': {
        ...
        fields: {
          source: 'Source slot in the epoch\'s catalog, 0 to `sourceCountAt(epochId) - 1`.',
          recipe: 'Global recipe id at that slot, 0 to `RECIPE_COUNT - 1`; see README [Replay and verification](README.md#replay-and-verification) for the recipes.',
          airnode: 'Signer of that slot in the catalog in force for the epoch.',
          selector: '`keccak256(abi.encode(SELECT_DOMAIN, catalogHash, epochId, anchor))`; the slot is `(selector mod count + attempt) mod count`.',
          queryHash: '`keccak256` of `canonicalRequest`.',
          canonicalRequest: 'Canonical request string of the recipe, the value of `recipeRequest(recipe)`.',
        },
      },
      'EpochEntropy.Attestation': {
        ...
        fields: {
          ...
          data: 'Signed response bytes, 1 to 128 bytes, in the exact format of the recipe.',
    ...
    functions: [
      {
        ...
        items: {
          ...
          catalogAt: {
            caller: 'Anyone (view)',
            src: '96-103, 109-113',
            text: 'The catalog an epoch selects and commits with: its hash and, in slot order, the recipe id and signer of each source. It is the latest scheduled version whose `fromEpoch` is at or below `epochId`, otherwise the initial catalog (recipes 0 to 3 with the initial signers and `catalogHash()`). Replay needs this, not the initial getters.',
          },
          sourceCountAt: {
            caller: 'Anyone (view)',
            src: '105-108',
            text: 'Number of sources in the epoch\'s catalog, 1 to `MAX_SOURCES`; selection attempts run from 0 to this count minus one.',
          },
          recipeRequest: {
            caller: 'Anyone (pure)',
            src: '158-169',
            text: 'The canonical request string of a recipe id; its `keccak256` is the query hash the recipe\'s signer attests.',
      ...
      },
      {
        ...
        items: {
          firstEpochStart: { src: '31, 63', text: 'First block of epoch 1: the initialization block plus 200.' },
          committer: { src: '30, 177', text: 'Address allowed to publish epochs. The coordinator also pays the keeper share to it at each acceptance.' },
          catalogHash: { src: '32, 64', text: 'Initial catalog hash, bound into `protocolConfigurationHash`. Never changes; `catalogAt` gives the catalog of an epoch.' },
          epochAnchors: { src: '42, 124-127', text: 'Checkpointed anchor of an epoch (hash of block `epochStart - 1`); zero until a request, `checkpointEpoch` or publication stores it.' },
          hyperliquidSigner: { src: '25', text: 'Initial-catalog signer of recipe 0 (Hyperliquid BTC volume). Never changes; see `catalogAt`.' },
          ethereumBlockSigner: { src: '27', text: 'Initial-catalog signer of recipe 1 (dRPC Ethereum block hash). Never changes; see `catalogAt`.' },
          btcTradeSigner: { src: '28', text: 'Initial-catalog signer of recipe 2 (TickerLayer BTCUSD). Never changes; see `catalogAt`.' },
          ethTradeSigner: { src: '29', text: 'Initial-catalog signer of recipe 3 (TickerLayer ETHUSD). Never changes; see `catalogAt`.' },
        },
      },
      {
        ...
        items: {
          ...
          getEpochFallbackSelection: {
            caller: 'Anyone (view)',
            src: '138, 145-156',
            text: 'The source for attempt 0 to `sourceCountAt(epochId) - 1`; attempt n uses the slot n positions after the selected one, wrapping around the catalog.',
          ...
          commitEpoch: {
            caller: 'Committer',
            src: '170, 176-195',
            text: 'Publishes the packet of the selected source once per epoch, from the epoch start: checks that the attestation is not future-dated and at most 240 seconds old, that its data has the exact format of the slot\'s recipe, and that the slot\'s signer in the epoch\'s catalog signed it. Stores the record and emits the packet.',
      ...
      },
      {
        ...
        items: {
          ...
          ...ownershipFunctions('67-68'),
          initialize: {
            caller: 'Once, by `D20Proxy` at deployment',
            src: '58-65',
            text: 'Sets the four signers of the initial catalog (recipes 0 to 3), owner and committer. Epoch 1 starts 200 blocks after the initialization block.',
            emits: ['OwnershipTransferred', 'Initialized'],
      ...
      },
      {
        ...
        items: {
          ...
          MAX_SOURCES: { text: 'Most sources a catalog can list; the last fallback window then opens 180 blocks into the epoch.' },
          RECIPE_COUNT: { text: 'Number of recipe ids known to this implementation.' },
    ...
    events: [
      {
        ...
        items: {
          ...
          CatalogScheduled: {
            src: '55, 93',
            text: 'Recipes and signers scheduled for epochs from `fromEpoch`. A later `CatalogScheduled` emitted while this version has not taken effect replaces it, so when rebuilding catalogs from history drop replaced versions, or read `catalogAt(epochId)`.',
          },
        },
      },
      {
        title: 'Administration and upgrades',
        items: {
          CommitterChanged: { src: '54, 72', text: 'New publishing address and keeper-share recipient.' },
          ...ownershipEvents,
        },
      },
    ],
    errors: [
      ...
      },
      {
        title: 'Publication',
        items: {
          OnlyCommitter: { src: '50, 177', text: 'A commit from an address other than `committer()`.', response: 'Only the committer publishes.' },
          AlreadyCommitted: { src: '50, 178', text: 'The epoch already has a published packet.', response: 'Nothing to publish; read `getEpoch`.' },
          FallbackNotOpen: { src: '52, 179', text: '`block.number` is below `fallbackOpensAt(epochId, attempt)`; for attempt 0, before the epoch start.', response: 'Wait for the window.' },
          InvalidFallback: { src: '52, 141, 149, 173', text: 'An attempt at or above `sourceCountAt(epochId)`, or attempt 0 passed to `commitEpochFallback`.', response: 'Use attempts 1 to `sourceCountAt(epochId) - 1` for fallbacks.' },
          InvalidTime: { src: '50, 181', text: 'The attestation timestamp is in the future or more than `MAX_ATTESTATION_AGE` (240 seconds) before the publication block.', response: 'A saved packet is never refreshed; its requests expire and are refunded.' },
          InvalidData: { src: '50, 197-224', text: 'The signed data is empty, longer than 128 bytes, or not in the exact format of the slot\'s recipe.', response: 'Publish only a validated response for the selected slot.' },
          InvalidSigner: { src: '50, 184', text: 'The signature does not recover to the slot\'s signer in the epoch\'s catalog.', response: 'Use `catalogAt(epochId)` for the expected signer.' },
          ...
          PacketTooLarge: { src: '51, 193', text: 'The encoded packet exceeds `MAX_PACKET_BYTES` (2048).', response: 'Not reachable with data of at most 128 bytes and a 65-byte signature.' },
        },
      },
      {
        title: 'Administration, initialization and upgrades',
        items: {
          InvalidConfig: {
            src: '49, 61, 71, 79, 83, 168',
            text: 'A zero signer or committer in `initialize`, a zero address in `setCommitter`, a `scheduleCatalog` with no or more than `MAX_SOURCES` recipes, mismatched lengths, a repeated or unknown recipe id or a zero signer, or an unknown id passed to `recipeRequest`.',
            response: 'Use non-zero addresses and 1 to 10 distinct known recipe ids with one signer each.',
          },
          ...upgradeErrors,
          RenounceDisabled: { ...upgradeErrors.RenounceDisabled, src: '51, 68' },
        },
      },
    ],
  },
