  ...
  },
  {
    ...
    functions: [
      ...
      },
      {
        ...
        items: {
          firstEpochStart: { src: '40, 82', text: 'First block of epoch 1: the initialization block plus 200.' },
          committer: { src: '39, 276', text: 'Primary publishing address. The coordinator pays the keeper share to it at each acceptance, also for epochs a backup committer published.' },
          isBackupCommitter: { src: '117', text: 'Whether an address may publish epochs besides `committer()`.' },
          backupCommitterCount: { src: '62, 106-116', text: 'Number of allowed backup committers, at most `MAX_BACKUP_COMMITTERS`.' },
          catalogHash: { src: '41, 83', text: 'Initial catalog hash, bound into `protocolConfigurationHash`. Never changes; `catalogAt` gives the catalog of an epoch.' },
          epochAnchors: { src: '53, 236-239', text: 'Checkpointed anchor of an epoch (hash of block `epochStart - 1`); zero until a request, `checkpointEpoch` or publication stores it.' },
      ...
      },
      {
        ...
        items: {
          ...
          setBackupCommitter: {
            caller: 'Owner',
            src: '103-116',
            text: 'Allows or removes a backup committer: a separate wallet that may call `commitEpoch` and `commitEpochFallback` under exactly the committer\'s rules, for example a follower keeper that takes over while the primary keeper is down. It earns nothing; the coordinator keeps paying the keeper share to `committer()`. Reverts for the zero address, for allowing the current committer, for a call that does not change the address\'s status, and for more than `MAX_BACKUP_COMMITTERS` (4).',
            emits: ['BackupCommitterSet'],
            errors: ['OwnableUnauthorizedAccount', 'InvalidConfig'],
          },
          ...ownershipFunctions('96-97'),
          initialize: {
            caller: 'Once, by `D20Proxy` at deployment',
            src: '77-85',
    ...
    events: [
      ...
      },
      {
        title: 'Administration and upgrades',
        items: {
          CommitterChanged: { src: '71, 101', text: 'New primary publishing address and keeper-share recipient.' },
          BackupCommitterSet: { src: '74, 115', text: '`account` may now publish epochs (`allowed` true) or no longer may (`allowed` false).' },
          ...ownershipEvents,
          Initialized: {
    ...
    errors: [
      ...
      },
      {
        title: 'Publication',
        items: {
          OnlyCommitter: { src: '66, 276', text: 'A commit from an address that is neither `committer()` nor an allowed backup committer.', response: 'Only the committer and backup committers publish; check `isBackupCommitter`.' },
          AlreadyCommitted: { src: '66, 277', text: 'The epoch already has a published packet.', response: 'Nothing to publish; read `getEpoch`.' },
          FallbackNotOpen: { src: '68, 278', text: '`block.number` is below `fallbackOpensAt(epochId, attempt)`; for attempt 0, before the epoch start.', response: 'Wait for the window.' },
          InvalidFallback: { src: '68, 253, 261, 272', text: 'An attempt at or above `sourceCountAt(epochId)`, or attempt 0 passed to `commitEpochFallback`.', response: 'Use attempts 1 to `sourceCountAt(epochId) - 1` for fallbacks.' },
          InvalidTime: { src: '66, 280', text: 'The attestation timestamp is in the future or more than `MAX_ATTESTATION_AGE` (240 seconds) before the publication block.', response: 'A saved packet is never refreshed; its requests expire and are refunded.' },
          InvalidData: { src: '66, 281-282', text: 'The signed data does not match the data template of the slot\'s recipe exactly, which includes data longer than `MAX_DATA_BYTES` (128).', response: 'Publish only a response that matches the selected recipe\'s template, unmodified.' },
          InvalidSigner: { src: '66, 284', text: 'The signature does not recover to the slot\'s signer in the epoch\'s catalog.', response: 'Use `catalogAt(epochId)` for the expected signer.' },
          ...
          PacketTooLarge: { src: '67, 293', text: 'The encoded packet exceeds `MAX_PACKET_BYTES` (2048).', response: 'Not reachable: the recipe and data bounds keep every packet within `MAX_PACKET_BYTES`.' },
        },
      },
      {
        ...
        items: {
          ...
          InvalidConfig: {
            src: '65, 80, 92, 100, 107, 109, 135, 191, 195',
          ...
          RenounceDisabled: { ...upgradeErrors.RenounceDisabled, src: '67, 97' },
        },
      },
    ],
  },
